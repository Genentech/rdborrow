...
DID_EC_AIPW_bootstrap = function(data,
  ...
  if (model_form_piA == ""){
    ...
    suppressWarnings({piAX = predict(piA, newdata = df, type = "response")}) 
  ...
  suppressWarnings({Y0 = data.frame(sapply(1:T_follow, function(x){predict(model_list_ext[[x]], newdata = df)}))})
