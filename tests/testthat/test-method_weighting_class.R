test_that("setup_method_weighting returns default object", {
  obj <- suppressWarnings(setup_method_weighting())
...
test_that("setup_method_weighting inherits base validation", {
  expect_warning(expect_error(setup_method_weighting(method_name = 123)))
  expect_warning(expect_error(setup_method_weighting(bootstrap_flag = "yes")))
  expect_warning(expect_error(setup_method_weighting(bootstrap_obj = list())))
})

test_that("setup_method_weighting validates weighting-specific args", {
  expect_warning(expect_error(setup_method_weighting(optimal_weight_flag = "yes")))
  expect_warning(expect_error(setup_method_weighting(wt = "heavy")))
  expect_warning(expect_error(setup_method_weighting(model_form_piS = 123)))
})
