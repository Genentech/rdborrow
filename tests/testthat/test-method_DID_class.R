...
test_that("setup_method_DID inherits base validation", {
  expect_error(setup_method_DID(method_name = 123))
  expect_error(setup_method_DID(bootstrap_flag = "yes"))
  expect_error(setup_method_DID(bootstrap_obj = list()))
})

test_that("setup_method_DID validates DID-specific args", {
  expect_error(setup_method_DID(model_form_piS = 123))
  expect_error(setup_method_DID(model_form_piA = NA))
  expect_error(setup_method_DID(model_form_mu0_ext = 123))
})
